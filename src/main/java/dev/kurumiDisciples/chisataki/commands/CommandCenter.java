...
public class CommandCenter extends ListenerAdapter {
	...
	private static final Map<String, SlashCommand> slashCommandsMap = new HashMap<>();
	private static final Map<String, UserCommand> userCommandsMap = new HashMap<>();
	private static final Map<String, MessageCommand> messageCommandsMap = new HashMap<>();
	...
	private static List<CommandData> unwrapCommands() {
	...
	public static void addCommands(JDA jda) {
		// To uncomment once the code block will be refactored
	...
	private void handleSlashInteraction(SlashCommandInteractionEvent event) {
		...
		if (!command.isAllowed(event)) {
			event.getHook().editOriginal(command.getErrorMessage()).queue();
			return;
		}
